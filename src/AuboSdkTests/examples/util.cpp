...
void Util::printWaypoint(aubo_robot_namespace::wayPoint_S &wayPoint)
{
    ...
    std::cout<<"RX:"<<tempRpy.rx*180/M_PI<<"  RY:"<<tempRpy.ry*180/M_PI<<"   RZ:"<<tempRpy.rz*180/M_PI<<std::endl;
...
{
    std::cout << std::endl << "----------关节状态信息-------" << std::endl;
    ...
    for(int i=0; i<len; i++)
    {
        ...
        std::cout << "关节角: " << jointStatus[i].jointPosJ*180.0/M_PI << " ";
