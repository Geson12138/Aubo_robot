...
class Auboi5Robot:
    ...
    def set_gripper_init(self):
        """
        * FUNCTION:    set_gripper_init
        * DESCRIPTION: 设置机械臂末端电动夹爪初始化
        * INPUTS:      
        * OUTPUTS:
        * RETURNS:     成功返回: RobotError.RobotError_SUCC
        *              失败返回: 其他
        * NOTES:
        ...
        # 设置 IO 状态
        io_type = 5
        io_name = "U_DO_00"; io_value = 0 # DO_0: 0
        result_1 = self.set_board_io_status(io_type, io_name, io_value)
        io_name = "U_DO_01"; io_value = 0 # DO_1: 0
        result_2 = self.set_board_io_status(io_type, io_name, io_value)
        time.sleep(0.5)
        if result_1 != RobotErrorType.RobotError_SUCC or result_2 != RobotErrorType.RobotError_SUCC:
            logger.info("设置 IO 状态失败。错误码：{0}。".format(result_1))
        else: 
            logger.info("夹爪初始化")

    def set_gripper_open(self):
        """
        * FUNCTION:    set_gripper_open
        * DESCRIPTION: 设置机械臂末端电动夹爪打开
        * INPUTS:      
        * OUTPUTS:
        * RETURNS:     成功返回: RobotError.RobotError_SUCC
        *              失败返回: 其他
        * NOTES:
        ...
        io_name = "U_DO_00"; io_value = 0 # DO_0: 0
        result_1 = self.set_board_io_status(io_type, io_name, io_value)
        io_name = "U_DO_01"; io_value = 1 # DO_1: 0
        result_2 = self.set_board_io_status(io_type, io_name, io_value)
        time.sleep(0.5)
        if result_1 != RobotErrorType.RobotError_SUCC or result_2 != RobotErrorType.RobotError_SUCC:
            logger.info("设置 IO 状态失败。错误码：{0}。".format(result_1))
    ...
    def set_gripper_close(self):
        """
        * FUNCTION:    set_gripper_close
        * DESCRIPTION: 设置机械臂末端电动夹爪闭合
        * INPUTS:      
        * OUTPUTS:
        * RETURNS:     成功返回: RobotError.RobotError_SUCC
        *              失败返回: 其他
        * NOTES:
        ...
        # 设置 IO 状态
        io_type = 5
        io_name = "U_DO_00"; io_value = 1 # DO_0: 0
        result_1 = self.set_board_io_status(io_type, io_name, io_value)
        io_name = "U_DO_01"; io_value = 0 # DO_1: 0
        result_2 = self.set_board_io_status(io_type, io_name, io_value)
        time.sleep(0.5)
