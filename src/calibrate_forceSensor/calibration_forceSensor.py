...
'''
FUNCTION: 读取原始力传感器数据 5ms/次
'''
# 定义报文头和尾
PACK_BEGIN = "<PACK_BEGIN"
PACK_END = "PACK_END>"
force_data = np.zeros(6) # fx fy fz Mx My Mz 力传感器数据

def read_force_data(sock):

    global force_data,running
    buffer = b""
...
'''
def online_calibrate_force(robot):
    ...
    with open(json_filename, 'a') as json_file: # 追加模式
        ...
        while True:
            ...
            '''
            logger.info('获取的力传感数值: {}'.format(force_data))
            ...
            logger.info('获取的末端姿态矩阵: {}'.format(inv_rotation_matrix))
...
验证力传感器标定结果
'''
def validate_calibrate_result(robot):
...
def plot_external_force():
    ...
    ax1.set_xlim(0, 25)  # 显示最近20秒的数据
    ax1.set_ylim(-20, 20)
    ax1.set_ylabel('F/N')
    ax1.legend()
    
    ax2.set_xlim(0, 25)  # 显示最近20秒的数据
    ax2.set_ylim(-10, 10)
...
if __name__ == "__main__":
    ...
    # 单开一个线程读取原始力传感器数据
    read_thread = threading.Thread(target=read_force_data, args=(sock,))
    read_thread.start()
    ...
    robot.connect('192.168.26.103', 8899)
    logger.info("连接机器人成功")
    ...
    # online_calibrate_force(robot) # 在线执行力传感器标定
    ...
    validate_thread = threading.Thread(target=validate_calibrate_result, args=(robot,))
